...
<script>
	...
	user.put("oid", tempUser.getPersistInfo().getObjectIdentifier().getId());
	user.put("name", tempUser.getFullName());
	user.put("org", tempUser.getOrganizationName());
	user.put("email", tempUser.getEMail());
	user.put("regDate", tempUser.getCreateTimestamp());
	
	function onLoad() {
		userGrid = new dhtmlXGridObject("userTable");
		userGrid.setImagePath("/Windchill/extcore/dhtmlx/imgs/");
		userGrid.setHeader("이름,oid,이메일,조직,가입일");
		userGrid.setColumnIds("name,oid,email,org,regDate");
		userGrid.setInitWidthsP("20,10,25,15,30");
		userGrid.setColAlign("center,left,left,left,left");
		userGrid.setColTypes("ro,ro,ro,ro,ro");
		userGrid.enableResizing("false,false,false,false,false");
		userGrid.setSkin("dhx_skyblue");
		userGrid.setColSorting("str,str,str,str,str");
		userGrid.enableResizing("false,false,false,false,false");
	...
	function getUsers() {
		$.ajax({
			url : "/Windchill/servlet/dgt/user/getUsers/qs",
			type : "POST",
			dataType : "json",
			data : {},
