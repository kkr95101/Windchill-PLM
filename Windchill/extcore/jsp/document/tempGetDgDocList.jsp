...
<script>
	...
	function onLoad() {
		...
		docGrid.setHeader("#master_checkbox,이름,내용,회원이름,회원나이,이터레이션,idA2A2,사용자성별,사용자조직");
		...
		docGrid.enableResizing("false,false,false,false,false,false,false,false,false");
	...
	function apply(){
		...
		if(checkobjs == null || checkobjs == ""){
			...
			for(let i=0; i<tempArr.length;i++){
				...
				alert(returnArr[i][0]);
				returnArr[i][1] = docGrid.getRowAttribute(tempArr[i],"description"); 
				returnArr[i][2] = docGrid.getRowAttribute(tempArr[i],"displayName");
			}
...
<body>
	...
	<input type="button" value="닫기" onclick="window.close()"> <!-- 윈도우창 닫기 -->
	</form>
